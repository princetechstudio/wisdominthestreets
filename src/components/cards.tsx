...
export function CoverArt({ ep, className = "" }: { ep: Episode; className?: string }) {
  return (
    ...
    >
      {ep.imageUrl && <img src={ep.imageUrl} alt="" className="absolute inset-0 h-full w-full object-cover opacity-75" loading="lazy" />}
      {/* street grid pattern */}
...
export function NewsletterForm({ compact = false }: { compact?: boolean }) {
  ...
  const submit = async (e: FormEvent) => {
    ...
    setStatus("loading");
    const endpoint = import.meta.env.VITE_NEWSLETTER_ACTION;
    if (!endpoint || endpoint.includes("YOUR_")) {
      setStatus("idle");
      setError("Newsletter signup is not configured yet. Please check back soon.");
      toast("Newsletter setup is incomplete", "error");
      return;
    }
    ...
    try {
      ...
      setStatus("done");
      try {
        localStorage.setItem("wits-newsletter", email.trim());
      } catch (error) {
        console.warn("Newsletter preference could not be saved locally", error);
      }
      toast("You're on the list — wisdom lands Friday ✦", "success");
    } catch (error) {
      console.error("Newsletter signup failed", error);
      setStatus("idle");
      setError("We couldn't complete signup. Please try again later.");
      toast("Newsletter signup failed", "error");
    }
  };
