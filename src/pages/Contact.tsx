...
export default function Contact() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (!endpoint || endpoint.includes("YOUR_FORM_ID")) {
      setStatus("idle");
      setSubmitError("Contact forms are not configured yet. Please use WhatsApp or email while the site owner finishes setup.");
      toast("Contact form setup is incomplete", "error");
      return;
    }
    ...
    try {
      ...
      setStatus("idle");
      setSubmitError("We couldn't send that message. Please try WhatsApp or email instead.");
          ...
          <div className="rounded-xl border border-line bg-panel p-6 sm:p-8">
            {status === "sent" ? (
              ...
              <form onSubmit={submit} noValidate>
                ...
                {submitError && <p className="mt-3 text-sm text-ember" role="alert">{submitError}</p>}
              </form>
            )}
          </div>
        </Reveal>
            ...
            <div className="divide-y divide-[var(--wits-line)] rounded-xl border border-line bg-panel">
              ...
              <div className="flex items-center gap-4 p-5">
                ...
                <span>
                  ...
                  <span className="text-sm font-medium text-ink">Pokuase-Mayera, Accra, Ghana</span>
                </span>
              </div>
              <div className="flex items-center gap-4 p-5">
