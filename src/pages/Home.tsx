...
export default function Home() {
  ...
  const { episodes, quotes, media } = useCms();
  ...
  const featured = episodes.find((episode) => episode.id === FEATURED_EPISODE.id) ?? FEATURED_EPISODE;
  ...
  return (
    <>
        ...
        <div className="relative mx-auto grid w-full max-w-7xl flex-1 items-center gap-14 px-4 pb-16 pt-28 sm:px-6 lg:grid-cols-[1.15fr_0.85fr] lg:px-8 lg:pt-32">
          ...
          <div>
            ...
            >
              ...
              street level, where traders, drivers and aunties drop more truth in five minutes than most boardrooms do in five hours.
            </motion.p>
      ...
      {media.length > 0 && (
        <section className="mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8">
          <SectionHead kicker="From the publishing desk" title="THE LATEST DROP" />
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {media.slice(0, 6).map((item) => (
              <article key={item.id} className="overflow-hidden rounded-xl border border-line bg-panel">
                {item.type === "video" ? (
                  <a href={item.url} target="_blank" rel="noreferrer" className="block aspect-video bg-black">
                    <img src={item.thumbnailUrl || IMAGES.street} alt="" className="h-full w-full object-cover opacity-80" loading="lazy" />
                  </a>
                ) : (
                  <img src={item.url} alt={item.title} className="aspect-video h-full w-full object-cover" loading="lazy" />
                )}
                <div className="p-5"><h3 className="font-head text-lg font-semibold text-ink">{item.title}</h3>{item.description && <p className="mt-2 text-sm text-mute">{item.description}</p>}</div>
              </article>
            ))}
          </div>
        </section>
          ...
          <Reveal delay={0.1}>
            <div className="group relative flex h-full min-h-[300px] flex-col justify-end overflow-hidden rounded-xl border border-line p-8 transition hover:border-ember/50">
              ...
              <div className="relative">
                ...
                <p className="mt-3 max-w-sm text-sm leading-relaxed text-mute">
                  Chairs in a circle, a mic in the middle. First open-air recording in Pokuase-Mayera — the community asks the questions.
                </p>
                <button
